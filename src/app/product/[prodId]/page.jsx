                ...
                <div className="flex flex-col justify-center md:flex-row w-full gap-4">
                    ...
                    <div className="flex flex-col h-auto w-full md:w-1/3 md:px-4 gap-1 text-gray-700">
                        <p className="text-green-600 text-sm md:text-base font-semibold mt-2 mx-0.5">
                            {Math.round(100 - (item?.discountedPrice / item?.price) * 100)}% Discount
                        </p>
                        <p className="text-2xl md:text-3xl font-bold ">{item?.name}
                        </p>

                        <div className="my-3 mx-1">

                            <p className="text-slate-400 text-sm mx-1">People interested: 3</p> {/* to be updated */}

                            <ul className="text-base md:text-lg my-3 mx-1">
                                <p className="font-bold">Breed: <span className="font-normal text-slate-600 ">{item.breed}</span></p>
                                <p className="font-bold">Height: <span className="font-normal text-slate-600">{item.height} cm</span></p>
                                <p className="font-bold">Weight: <span className="font-normal text-slate-600">{item.weight} kg</span></p>
                                <p className="font-bold">Age: <span className="font-normal text-slate-600">{item.age} years</span></p>
                                <p className="font-bold">Sex: <span className="font-normal text-slate-600">{item.sex}</span></p>
                                <p className="font-bold">Type: <span className="font-normal text-slate-600">{item.type}</span></p>
                                <p className="font-bold">Nature: <span className="font-normal text-slate-600">{item.nature}</span></p>
                                <p className="font-bold">Availability: <span className={`font-normal text-slate-600 ${item.availability === "In Stock" ? "text-green-600" : "text-red-600"}`}>{item.availability} </span></p>
                            </ul>
                        </div>
                        <div className="flex flex-col gap-5">

                            <div className="text-[32px] md:text-[36px] font-mono text-orange-600">
                            ...
                            <div>
                                <button
                                    className="bg-[#8a5e2f] hover:bg-[#644321] text-white py-2 px-4 rounded-full w-full text-base md:text-lg"
                                >
                                    Buy Now
                                </button>
                                <button
